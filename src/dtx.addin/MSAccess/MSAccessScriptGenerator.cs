    ...
    public class MSAccessScriptGenerator : IScriptGenerator
    {
        ...
        public string InsertRow(TableSchema schema, RowData row)
        {
            ...
            foreach (var fieldSchema in schema.Fields)
            {
                ...
                if (string.IsNullOrEmpty(row[fieldSchema.FieldName]) && fieldSchema.IsNullable)
                {
                    values.Add("NULL");
                }
                else
                {
                    ...
                    {
                        var strValue = "'" + row[fieldSchema.FieldName].Replace("'", "''") + "'";
                        values.Add(strValue);
                    }
                    ...
                    {
                        // Int, double, boolean...
                        values.Add(row[fieldSchema.FieldName]);
                    }
                }
            }
